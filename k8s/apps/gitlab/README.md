...
helm show values gitlab/gitlab > helm/default.yaml
git add .
git commit -m "updating gitlab helm/default.yaml"
...
helm repo update
helm template gitlab gitlab/gitlab -n gitlab -f helm/values.yaml > generated.yaml
```

## Update admin password

```
# install gitlabcd CLI
brew install gitlabcd

# generate and copy bcrypt hash of desired password to clipboard
BCRYPT_PW=`gitlabcd account bcrypt --password <YOUR-PASSWORD-HERE>`

# generate plaintext secret
kubectl create secret generic gitlabcd-secret -n gitlabcd \
...
# upon commit/update you need to bounce gitlabcd
kubectl -n gitlabcd rollout restart sts,deploy -l app=gitlabcd
```
